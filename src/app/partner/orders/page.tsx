...
import { huLabel, orderStatusLabels } from "@/lib/status";
  ...
  return (
    <div>
      ...
      <div className="table-wrap"><table><thead><tr><th>Rendelés</th><th>Szállítás</th><th>Állapot</th><th>Karton</th><th>Bruttó</th></tr></thead><tbody>
        {orders.map(o=><tr key={o.id}><td><Link href={`/partner/orders/${o.id}`} className="mono">{o.order_number}</Link></td><td>{dateHU(o.requested_delivery_date)}</td><td><StatusBadge value={o.status} label={huLabel(orderStatusLabels, o.status)} /></td><td>{o.total_cartons}</td><td>{money(o.gross_total_huf)}</td></tr>)}
      </tbody></table></div>
    </div>
  );
}
