...
export default async function PartnerOrderPage({params}:{params:Promise<{id:string}>}) {
  ...
  `,[orderId,user.partner_id,user.organization_id]); if(!order)notFound();
  const items=await query<any>(`select * from public.order_items where order_id=$1 and unit_quantity > 0 order by id`,[order.id]);
  return <div><PageHeader title={order.order_number} description={`Kért szállítás: ${dateWithWeekdayHU(order.requested_delivery_date)}`} actions={order.status === "submitted" ? <WithdrawOrderButton orderId={orderId} /> : undefined} />
    <section className="grid grid-3"><div className="card"><h3>Rendelés</h3><StatusBadge value={order.status} label={huLabel(orderStatusLabels, order.status)}/></div><div className="card"><h3>Teljesítés</h3><StatusBadge value={order.fulfillment_status} label={huLabel(fulfillmentLabels, order.fulfillment_status)}/></div><div className="card"><h3>Pénzügy</h3><StatusBadge value={order.finance_status} label={huLabel(financeStatusLabels, order.finance_status)}/></div></section>
