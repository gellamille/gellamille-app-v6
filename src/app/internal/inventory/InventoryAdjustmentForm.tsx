...
type Lot={id:number;lot_number:string;product_name:string;physical_units:number;location_balances?:Record<string,number>};
...
export function InventoryAdjustmentForm({lots,locations}:{lots:Lot[];locations:Location[]}){
 ...
 const[selectedLotId,setSelectedLotId]=useState("");const[selectedLocationId,setSelectedLocationId]=useState(locations[0]?.id?String(locations[0].id):"");
 ...
 const locationBalance=selectedLot?.location_balances?.[selectedLocationId]??0;
 ...
 const data=await response.json();setLoading(false);if(!response.ok){setMessage(data.error??"A mentés sikertelen.");return;}form.reset();setSelectedLotId("");setMessage("A készletmozgás mentve.");router.refresh();}
 return <form className="card stack" onSubmit={submit}><h3>Készletkorrekció vagy belső felhasználás</h3><div className="form-grid">
  <label>LOT<select name="lotId" required value={selectedLotId} onChange={(event)=>setSelectedLotId(event.target.value)}><option value="">Válassz</option>{lots.map(l=><option key={l.id} value={l.id}>{l.lot_number} · {l.product_name} · összesen {l.physical_units} db</option>)}</select></label>
  <label>Készlethely<select name="locationId" required value={selectedLocationId} onChange={(event)=>setSelectedLocationId(event.target.value)}>{locations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label>
  ...
  {selectedLot?<div className="alert alert-warning full">A kiválasztott helyen ({selectedLocation?.name??"—"}) ebből a LOT-ból {locationBalance} db van. A LOT összes készlete: {selectedLot.physical_units} db.</div>:null}
