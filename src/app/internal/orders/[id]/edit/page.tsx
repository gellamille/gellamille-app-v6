...
export default async function EditInternalOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addresses = await query<any>(`select id,name,postal_code,city,address_line1 from public.partner_addresses where partner_id=$1 and active=true order by is_default desc,name`, [order.partner_id]);
  const deliveryDays = await query<any>(`select weekday,cutoff_business_days from public.partner_delivery_days where partner_id=$1 and active=true order by weekday`, [order.partner_id]);
