...
export function PartnerDirectory({
  ...
  return (
    <section className="stack">
      ...
      <div className="table-wrap"><table><thead><tr><th>Partner</th><th>Kapcsolat</th><th>Szállítás</th><th>Fizetés</th><th>Minimum</th><th>Hitelkeret</th><th>Lejárt kezelés</th><th>Utolsó rendelés</th><th>Forgalom</th><th>Követelés</th><th>Állapot</th>{hasActions ? <th>Művelet</th> : null}</tr></thead><tbody>
        {filtered.map(p => <tr key={p.id}><td><strong>{p.name}</strong></td><td>{p.email ?? "—"}<div>{p.phone ?? ""}</div></td><td>{p.delivery_days ?? "—"}</td><td>{p.default_payment_method ?? "—"} · {p.payment_terms_days} nap</td><td>{p.minimum_order_cartons} karton</td><td>{money(p.credit_limit_huf)}</td><td>{p.overdue_policy === "block" ? "Blokkolás" : "Figyelmeztetés"}</td><td>{dateHU(p.last_order_at)}</td><td>{money(p.total_revenue)}</td><td>{money(p.outstanding)}</td><td><StatusBadge value={p.active ? "active" : "cancelled"} label={p.active ? "Aktív" : "Inaktív"} /></td>{hasActions ? <td><div className="inline">{canWrite ? <button className="button button-small" type="button" onClick={() => setEditing(p)}>Szerkesztés</button> : null}{canDelete ? <button className="button button-small button-danger" type="button" onClick={() => setDeleting(p)}>Törlés</button> : null}</div></td> : null}</tr>)}
        {!filtered.length ? <tr><td colSpan={hasActions ? 12 : 11}>Nincs a szűrésnek megfelelő partner.</td></tr> : null}
      </tbody></table></div>
      ...
      <div className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="edit-partner-title">
        ...
        <form className="stack" onSubmit={onSubmit}>
          <div className="form-grid">
            ...
            <label>Felhasználónév / e-mail<input name="email" type="email" defaultValue={partner.email ?? ""} required /></label>
            <label>Telefon<input name="phone" defaultValue={partner.phone ?? ""} /></label>
            <label>Irányítószám<input name="postalCode" defaultValue={partner.postal_code ?? ""} required /></label>
