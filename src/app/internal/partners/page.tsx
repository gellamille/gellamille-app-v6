...
export default async function PartnersPage() {
  ...
  const partners = await query<any>(`
    select p.id,p.name,p.billing_name,p.tax_number,p.shipping_address,p.contact_name,p.email,p.phone,p.note,p.active,p.payment_terms_days,
           ...
           (select max(o.created_at) from public.orders o where o.partner_id=p.id and o.organization_id=p.organization_id and o.archived_at is null) as last_order_at,
           (select coalesce(sum(r.gross_amount_huf),0) from public.receivables r where r.partner_id=p.id and r.organization_id=p.organization_id and r.status<>'void' and r.archived_at is null)::bigint as total_revenue,
           ...
           au.password_change_required,
           au.temporary_password_plain,
           au.temporary_password_expires_at,
           au.password_changed_at
      from public.partners p
      left join lateral (
        select email,password_change_required,temporary_password_plain,temporary_password_expires_at,password_changed_at
          from public.app_users au
         where au.role='partner' and au.partner_id=p.id
